...
def build_parser():
	...
	# Embeddings
	parser.add_argument('-embedding', type=str, default='roberta', choices=['bert', 'roberta', 'word2vec', 'random','combine_char_roberta'], help='Embeddings')
	...
	parser.add_argument('-emb_name', type=str, default='roberta-base', choices=['bert-base-uncased', 'roberta-base',"combine_char_roberta"], help='Which pre-trained model')
	...
	parser.add_argument('-char_hidden_size', type=int, default=768, help='It will decide the hidden state dimension of char embedding model')
	parser.add_argument('-char_embedding_size', type=int, default=512, help='Embedding matrix for char')
	parser.add_argument('-char_nlayer', type=int, default=1, help='It will decide the hidden state dimension of char embedding model')
	parser.add_argument('-char_dropout', type=float, default=0.1 ,help='It will decide the hidden state dimension of char embedding model')

	parser.add_argument('-char_cell_type', type=str, default='gru', help='RNN cell for character encoder , default: gru')

	parser.add_argument('-char_embed', dest='use_char_embed', action='store_true', help='Operate in adding more information')
	parser.add_argument('-no-char_embed', dest='use_char_embed', action='store_false', help='Operate in normal mode')
	parser.set_defaults(use_char_embed=False)

	parser.add_argument('-num_embed', dest='use_num_embed', action='store_true', help='Operate in adding more information')
	parser.add_argument('-no-num_embed', dest='use_num_embed', action='store_false', help='Operate in normal mode')
	parser.set_defaults(use_num_embed=False)

	parser.add_argument('-num_alpha', type=float, default=0.5, help='1-fraction of embedding value need to be added to roberta')
	
	parser.add_argument('-join_roberta_num_with_layer', dest='use_layer_add_num', action='store_true', help='Operate in adding more information')
	parser.add_argument('-Dont_join_roberta_num_with_layer', dest='use_layer_add_num', action='store_false', help='Operate in normal mode')
	parser.set_defaults(use_layer_add_num=False)
	# combining both layer embedding
...
# 	parser = argparse.ArgumentParser(description='Run Single sequence model for hyper parameter tunning')
...
# 	# LSTM parameters
# 	parser.add_argument('-emb2_size', nargs="*", default=tune.choice([16,32,64,128]), help='Embedding dimensions of inputs')
# 	parser.add_argument('-cell_type', type=str, default='lstm', help='RNN cell for encoder and decoder, default: lstm')
...
# 	parser.add_argument('-attn_type', type=str, default='general', help='Attention mechanism: (general, concat), default: general')
# 	parser.add_argument('-hidden_size', type=int, default=tune.choice([128,256,512]) ,help='Number of hidden units in each layer')
# 	parser.add_argument('-depth', type=int, default=1, help='Number of layers in each encoder and decoder')
# 	parser.add_argument('-dropout', type=float, default=tune.choice([0.0,0.1,0.2]) , help= 'Dropout probability for input/output/state units (0.0: no dropout)')
# 	parser.add_argument('-max_length', type=int, default=100, help='Specify max decode steps: Max length string to output')
# 	parser.add_argument('-init_range', type=float, default=0.08, help='Initialization range for seq2seq model')
# 	parser.add_argument('-bidirectional', dest='bidirectional', action='store_true', help='Bidirectionality in LSTMs')
# 	parser.add_argument('-no-bidirectional', dest='bidirectional', action='store_false', help='Bidirectionality in LSTMs')
# 	parser.set_defaults(bidirectional=True)
# 	parser.add_argument('-lr', type=float, default=tune.loguniform(1e-5,1e-1), help='Learning rate')
...
# 	# parser.set_defaults(train_word2vec=True)
# 	parser.add_argument('-emb1_size', type=int, default=768, help='Embedding dimensions of inputs')
# 	parser.add_argument('-emb_name', type=str, default='combine_char_roberta', choices=['bert-base-uncased', 'roberta-base',"combine_char_roberta"], help='Which pre-trained model')
# 	# parser.add_argument('-bert_size', type=int, default = 768, help = 'Size of BERT\'s last layer representations')
# 	parser.add_argument('-emb_lr', type=float, default=tune.loguniform(1e-5,1e-1), help='Larning rate to train embeddings')
# 	parser.add_argument('-freeze_emb', dest='freeze_emb', action='store_true', help='Freeze embedding weights')
# 	parser.add_argument('-no-freeze_emb', dest='freeze_emb', action='store_false', help='Train embedding weights')
# 	parser.set_defaults(freeze_emb=False)
...
# 	## char Embedding parameters
# 	parser.add_argument('-char_hidden_size', type=int, default=tune.choice([256,512,768]), help='It will decide the hidden state dimension of char embedding model')
# 	parser.add_argument('-char_embedding_size', type=int, default=tune.choice([256,512]), help='Embedding matrix for char')
# 	parser.add_argument('-char_nlayer', type=int, default=tune.choice([1,2]), help='It will decide the hidden state dimension of char embedding model')
# 	parser.add_argument('-char_dropout', type=int, default=tune.choice([0.0,0.1,0.2]),help='It will decide the hidden state dimension of char embedding model')
# 	parser.add_argument('-char_cell_type', type=str, default='gru', help='RNN cell for character encoder , default: gru')
