...
from src.combine_char_word_embed import combine_char_roberta
...
class Seq2SeqModel(nn.Module):
        def __init__(self, config, voc1, voc2, device, logger, num_iters, EOS_tag='</s>', SOS_tag='<s>'):
                ...
                if self.config.embedding == 'bert':
                        ...
                        # print(3,1,self.config.embedding)
                ...
                elif self.config.embedding == "combine_char_roberta" :
                        # print(self.config.embedding)
                        self.embedding1= combine_char_roberta(self.config,self.device)
                ...
                # print(3,2)
                self.logger.info('Building Encoders...')
        ...
        def _initialize_optimizer(self):
                ...
                #for name,param in self.embedding1.named_parameters(): print(name,param.requires_grad) check if parameter is getting updated in back propogation or not 
        ...
        def trainer(self, ques, nums, input_seq1, input_seq2, input_len1, input_len2, config, device=None ,logger=None):
                '''
                        Args:
                ...
                if self.config.embedding == 'bert' or self.config.embedding == 'roberta' or self.config.embedding == "combine_char_roberta" :
                        input_seq1, input_len1 = self.embedding1(ques,nums) 
                        #print(input_seq1)
        ...
        def greedy_decode(self, ques,nums, input_seq1=None, input_seq2=None, input_len1=None, input_len2=None, validation=False, return_probs = False):
                with torch.no_grad():
                        if self.config.embedding == 'bert' or self.config.embedding == 'roberta' or self.config.embedding == "combine_char_roberta":
                                input_seq1, input_len1 = self.embedding1(ques,nums)

                                
                                # print("input_seq1",input_seq1,input_seq1.size())
                                

                                input_seq1 = input_seq1.transpose(0,1)
                                sorted_seqs, sorted_len, orig_idx = sort_by_len(input_seq1, input_len1, self.device)
                                # print("sorted_seqs size",sorted_seqs.size())
...
def train_model(config,train_dataloader, val_dataloader, voc1, voc2, device, logger, writer= None):
        '''
                Add Docstring
        '''
        #config is passed as dictionary here
       
        
        #if isinstance(config,dict) and  config["hyper_param"]==True:
        #        for key in config.keys():
        #                config_dict_add[key]=config[key]
        #        config = Namespace(**config_dict_add)
                
        #print("check check")
        #print(voc2.get_id("</s>"))
        logger.info("going to build model")
        ...
        if config.hyper_param or  checkpoint == None:
        ...
        for epoch in range(1, config.epochs + 1):
                ...
                for data in train_dataloader:
                        ...
                        # for sent in text:
                        #         doc = nlp(sent)  
                        #         ques.append(doc._.coref_resolved)
                        ...
                        loss = model.trainer(ques, nums, sent1_var, sent2_var, input_len1, input_len2, config, device, logger)
                        ...
                        if config.show_train_acc:
                                ...
                                _, decoder_output, _ = model.greedy_decode(ques,nums,sent1_var, sent2_var, input_len1, input_len2, validation=True)
                ...
                #session.report({"val_acc_epoch":val_acc_epoch,"val_loss_epoch":val_loss_epoch,"val_bleu_epoch":val_bleu_epoch})
...
def run_validation(config, model, dataloader, voc1, voc2, device, logger, epoch_num):
        ...
        for data in dataloader:
                ...
                # print("sent1_var:",sent1_var.size())
                # print("sent1_var:",sent2_var.size())

                # print(sent1s)
                # print(sent2s)


                #print("data_per_batch_validation",len(ques))
                val_loss, decoder_output, decoder_attn = model.greedy_decode(ques,nums, sent1_var, sent2_var, input_len1, input_len2, validation=True)
                
                
                # calculate attention score
                if config.mode=="test":
                        embedding2 = RobertaEncoder(config.emb_name, device, config.freeze_emb)
                        tokens,sent=embedding2.all_token_with_padding_for_attention(ques)

                        def calculate_topk_words(attention_score , tokens , k):  
                                
                                # x=torch.Tensor(attention_score)
                                x=attention_score
                                y=torch.topk(x, k, dim=1, largest=True, sorted=True)
                                idx=y[1].squeeze().tolist()
                                topk=[tokens[i] for i in idx]
                        ...
                        for i in range(len(ques)):
                                mydict={}
                                mydict["problem"] = sent[i]
                                mydict["tokens"]  = tokens[i]
                                mydict["attention_score"] = [decoder_attn[j][i] for j in range(4)]
                                mydict["pred_equation"]  = decoder_output[i]
                                


                                for idx,dec_out in enumerate(decoder_output[i]):
                                        temp=decoder_attn[idx][i]
                                        # print(temp,idx,i)     
                                        mydict[dec_out]=[calculate_topk_words(temp,mydict["tokens"],3)]      # 3 is top3 from attention score,  it can be changed according to conveniance



                                # mydict["size_of_attention"] = decoder_attn[:][i].size()
                                answer.append(mydict)



                # Dimension of decoder attention : size_of_o/p_vocab * batch_size * 1 * max_length_input_sentence
                # print(tokens,sent,decoder_output)
                # print("decoder_output: ",decoder_output)
                # print("decoder_attn: ",decoder_attn)
                # print("shape of attention : ",len(decoder_attn),decoder_attn[0].size())
        ...
        if config.mode=="test":
        # save attention results to the output file
                torch.save(answer,config.outputs_path + "/attention_result.pt")
